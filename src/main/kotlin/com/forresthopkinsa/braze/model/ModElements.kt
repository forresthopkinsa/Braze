...
data class Mod(
    ...
    override var versions: List<IndexedModVersion>
) : Element<Mod, SimpleMod> {
    override fun simplify() = SimpleMod(slug, name, author, description, link, donate)
}

data class IndexedModVersion(
    override var name: String,
    ...
    var md5: String? = null,
    var size: Int? = null,
    var dependencies: List<SimpleModVersion>
) : IndexedVersion<ModVersion> {
    override fun simplify() = ModVersion(name, minForge, maxForge, md5, size, dependencies)
}

data class SimpleMod(
    override val slug: String,
